...
## 转录纠错（与"忠实原文"并行，不冲突）
逐字稿是语音转录的，常有字词错误（同音词、听错的专业术语/品牌名、被切错的词）。理解和总结时**应当在内部先把明显的转录错纠正过来**——这是还原"受访者本来要说的话"，和"凭空加信息"是两回事，不算幻觉。规则：
- **只改明显错**：仅当上下文让正确词**几乎无歧义**时才纠（如把听成 "span panel" 的 "spam panel"、把 "NEM" 转成 "name" 之类）。**任何拿不准的，保留原样，不要猜。**
- **只在内部纠正，不改原始 `.txt`**：原稿保持不动（它是存档源，且未纳入版本控制，覆盖不可退）。纠错只用于你的理解和写进 `.md` 的内容。
- **影响实质含义的纠错要标注**：若纠正的是关键术语/品牌/数字，在摘要里注明，便于核对，例如：`Span 面板（原文转录为 "spam panel"）`。无关紧要的小错（口语、语气词）直接按正确理解写即可，不必逐一标注。
- 边界：纠错只动"怎么把听错的词还原成对的词"，**不新增原文没有的事实、数字、结论**——后者仍受上面"忠实原文"约束。
